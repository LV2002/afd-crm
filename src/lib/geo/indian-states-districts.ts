/**
 * State list with district cascade for the lead form/filter's state→district
 * fields (docs/02-BUILD-PHASES.md Phase 1: "Port indianStatesDistricts.js
 * from v1 verbatim"). v1's actual source file wasn't available to port
 * byte-for-byte, so this is a fresh reference dataset covering every state
 * and union territory with its current districts — see docs/DECISIONS.md
 * for why an approximate re-creation was accepted here instead of blocking
 * on the original file.
 *
 * Deliberately fixed in code, not a `dropdown_options` row: India's states
 * and districts are geographic/political fact, not something an admin
 * would ever want to reconfigure per organisation — the same reasoning
 * CLAUDE.md gives for permission primitives and the object model.
 */
...
export const INDIAN_STATES_DISTRICTS: StateDistricts[] = [
  ...
  },
  {
    ...
    districts: [
      ...
      "Dharashiv", "Dhule", "Gadchiroli", "Gondia", "Hingoli", "Jalgaon", "Jalna", "Kolhapur",
  ...
  },
  {
    state: "Rajasthan",
    districts: [
      "Ajmer", "Alwar", "Balotra", "Banswara", "Baran", "Barmer", "Bharatpur", "Bhilwara",
      "Bikaner", "Bundi", "Chittorgarh", "Churu", "Dausa", "Deeg", "Dholpur", "Dungarpur",
      "Ganganagar", "Hanumangarh", "Jaipur", "Jaisalmer", "Jalore", "Jhalawar", "Jhunjhunu",
      "Jodhpur", "Karauli", "Kekri", "Khairthal-Tijara", "Kota", "Kotputli-Behror", "Nagaur",
...
export function districtsForState(state: string | null | undefined): string[] {
  return INDIAN_STATES_DISTRICTS.find((s) => s.state === state)?.districts ?? [];
}
